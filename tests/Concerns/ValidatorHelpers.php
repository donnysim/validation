...
namespace DonnySim\Validation\Tests\Concerns;

use DonnySim\Validation\Tests\Stubs\TranslationMessageResolver;
...
trait ValidatorHelpers
{
    protected function assertValidationFail(Validator $validator, string $key, string $message, int $errors = 1): void
    {
        ...
        self::assertCount($errors, $validator->getValidationMessages());
        $messages = $validator->getMessages($this->makeValidationMessageResolver());
        self::assertArrayHasKey($key, $messages);
        self::assertSame($message, $messages[$key]);
    }

    protected function makeValidationMessageResolver(): TranslationMessageResolver
    {
        return new TranslationMessageResolver([
