    ...
    public function resolve(array $messages): array
    {
        ...
        foreach ($messages as $message) {
            if (isset($this->messages[$message->getKey()])) {
                $result[$message->getEntry()->getPath()] = $this->makeReplacements(
                    $this->messages[$message->getKey()],
                    array_merge([
                        'path' => $message->getEntry()->getPath(),
                        'attribute' => $message->getEntry()->getPath(),
                    ], $message->getParams())
                );
            } else {
                $result[$message->getEntry()->getPath()] = 'missing message';
            }
        }
